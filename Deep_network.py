...
if __name__ == '__main__':
	...
	for hyperparameter in generator.iterate():
		...
		nb_neurons_1 = 32# nombre de neurones
		nb_neurons_2 = 16# nombre de neurones

		model = torch.nn.Sequential(
			torch.nn.Linear(data_train.shape[1],nb_neurons_1),
			torch.nn.ReLU(),
			torch.nn.Linear(nb_neurons_1,nb_neurons_2),
			torch.nn.ReLU(),
			torch.nn.Linear(nb_neurons_2,label_train.shape[1])
		)
		...
		torch.nn.init.uniform_(model[4].weight,w_min,w_max)
